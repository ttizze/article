...
Atlassian State of DevEx 2025（公式ウェビナー）では、次のように示されています：

- **68%がAIで週10時間以上"節約"**
- **50%が非コーディングの非効率で週10時間以上"喪失"**

AIで節約できる時間がある一方で、非コーディングの損失も大きいという自己申告が並んでいます。AtlassianのDevEx 2024レポートでも、**開発者の69%が週8時間以上の非効率**を感じていると報告されています。

では、どこが重いのか。研究や事例を見ると、**レビュー待ち**と**CI待ち**が大きな待ち時間を生みやすい領域として現れます。arXiv 2203.05048 の大規模コードレビュー分析では、初回応答待ちや承認後のマージ待ちといった遅延が明確に計測され、承認からマージまでの時間短縮だけでレビュー速度を29〜63%改善できる可能性が示唆されています。CI側でも、Uberの大規模CI運用研究（arXiv 2501.03440）でSubmitQueueの改善により**P95待ち時間が37%削減**されたと報告されており、待ち時間の最適化がボトルネック解消に直結しうることが示されています。

### 影響：レビュー負荷が熟練者に集中

レビューは「検証・レビュー」側のボトルネックが表面化しやすい領域です。以下は、その具体例として位置づけます。
...
レビュー負荷の増加は、PR量・サイズ・品質・プロセス要因など複数要因の影響を受けうることが示唆されます。

### 影響：AIレビューでも遅くなるケース

レビュー工程にAIを入れても、全体の流れが必ずしも速くならないことを示す例です。
...
コーディングの相対的な比重は下がりつつあります。しかし、ソフトウェアエンジニアリング全体のボトルネックはまだ解決していません。ボトルネックは「コーディング」から「仕様・検証・レビュー・運用」へ移動したのです。
...
### 例：バイブコーディング＝曖昧なまま進める典型

バイブコーディングでMVPは作れることが多い。しかし保守は難しくなりがちです：

- 実装の意図がわからない
- 壊れたら直せない

なぜか？正しい答えや判断基準が共有されていないからです。

### 要点：不確実性を下げる仕組みが必要

これはソフトウェアエンジニアリングがやってきたことです。

## 3. 解決原理：ソフトウェアエンジニアリングの役割
...
人数が増えると、同じ作業が重くなります。レビュー待ち、マージ競合、CI滞留、リリース調整などです。
...
- Atlassian State of DevEx 2025（公式ウェビナー: How AI Is Shaping Developer Productivity）
