...
では、どこが重いのか。研究や事例を見ると、**レビュー待ち**と**CI待ち**が大きな待ち時間を生みやすい領域として現れます。大規模なコードレビュー分析では、初回応答待ちや承認後のマージ待ちといった遅延が明確に計測され、承認からマージまでの時間短縮だけでレビュー速度を29〜63%改善できる可能性が示唆されています。CI側でも、Uberの大規模CI運用の事例ではSubmitQueueの待ち時間（P95）がGo/iOS/Androidで約31〜45%減少したと報告されており、待ち時間の最適化がボトルネック解消に直結しうることが示されています。
